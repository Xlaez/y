defmodule YCore.Social.FollowService do
  ...
  @spec follow(String.t(), String.t(), module()) ::
          {:ok, Follow.t()} | {:error, :cannot_follow_self} | {:error, :already_following} | {:error, term()}
  def follow(follower_id, followee_id, _repo) when follower_id == followee_id,
    do: {:error, :cannot_follow_self}

  def follow(follower_id, followee_id, repo) do
    repo.follow(follower_id, followee_id)
  end
